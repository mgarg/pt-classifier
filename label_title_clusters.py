...
USER_PROMPT_TEMPLATE = """I clustered ecommerce products using title-only embeddings.
...
def build_cluster_prompt(cluster_id, cluster_size, samples):
...
def create_labeling_prompts(
    clustered_file: str = "data/clusters/kmeans_template_vs_title_jina_20_clusters.csv",
    output_dir: str = "data/cluster_labeling/title_clusters",
    cluster_column: str = "title_cluster",
    ...
    jsonl_file = output_path / "title_cluster_labeling_prompts.jsonl"
    markdown_file = output_path / "title_cluster_labeling_prompts.md"
    samples_file = output_path / "title_cluster_labeling_samples.csv"
    ...
    with markdown_file.open("w") as f:
        f.write("# Title Cluster Labeling Prompts\n\n")
